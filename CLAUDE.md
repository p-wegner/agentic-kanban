...
**Per-project provider is now a first-class control (#925, Layer B).** Settings → Agent shows a **"Provider for this project"** dropdown whenever a project is active. It writes the provider as a single `fill` policy on the project's `board_strategy_<projectId>` Bullseye — NOT the global `provider` pref — so it takes effect immediately for that project's workspace launches and **never trips the divergence guard** (the write goes to the Bullseye, keeping the two sources in sync by construction). The simple control owns one policy id (`PROVIDER_DEFAULT_POLICY_ID = "provider-default"`, a single `fill`) and round-trips with the advanced **Provider policies** editor (Workflow tab) for multi-provider quota strategies — editing one reflects in the other. "Use global default" clears that policy so the project inherits the global Agent Profile. The global Agent Profile field is relabelled **"Agent Profile (global fallback)"** when a project is active, and the amber set-provider-default divergence warning is suppressed there (per-project divergence is now expected-by-design, not a misconfiguration). With NO active project, the global Agent Profile dropdown still sets the global default. Helpers: `setProviderFillPolicy` / `clearProviderFillPolicy` / `selectProviderFromPolicies` in `client/src/lib/strategy-targets.ts` (kept in lockstep with the server's `selectProviderFromStrategy`). This advances the "collapse the provider sources of truth" consolidation — one control, no guard needed for the per-project path. The `set-provider-default` skill remains for setting the GLOBAL default across all the still-separate sinks (settings prefs + `default_model`).
