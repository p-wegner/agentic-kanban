...
- **Multi-turn interactive chat**: Agent stdin stays open after initial prompt (via `keepAlive` flag in `agent.service.ts`). Follow-up messages are sent as JSONL `{"type":"user","content":"..."}` on stdin via `POST /:id/turn`. Turn state tracked server-side in `turnStates` map: `"processing"` (agent working) → `"waiting"` (result emitted, awaiting input). Result events in `broadcast()` detect `obj.type === "result" && isStdinOpen(sessionId)` to transition to `"waiting"`. Client derives `isWaitingForInput` by scanning messages backwards for a `result` event. `isSessionAlive` (= `activeSession !== null && isRunning`) distinguishes "agent alive" from "session ended". TerminalView shows "Turn complete — waiting for input" when `multiTurn` prop is true. Chat input disables during processing ("Agent is working..."), enables during waiting. Graceful stop: `closeStdin()` → 2s wait → `kill()`. The `stoppedByUser` set prevents the exit handler from overwriting the DB status set by `stopSession`. First launch (one-step workspace create, review sessions in `index.ts`) does NOT use multi-turn — `multiTurn` defaults to `undefined` for those callers, stdin closes immediately. Only `POST /:id/launch` defaults `body.multiTurn !== false` to true.
