...
2. **Client: `Failed to resolve entry for "@agentic-kanban/shared"`** — was: Vite resolved shared via the `import` condition → `dist/`, which a clean clone hasn't built (`dist/` gitignored, no `prepare`, `repairSharedIfNeeded` only restores wiped *source*). **Fixed**: `vite.config.ts` prepends the `"development"` condition in `serve` mode → shared resolves to `src/`, no pre-build (build still uses `dist`). Fallback if it recurs: `pnpm --filter @agentic-kanban/shared build`.
