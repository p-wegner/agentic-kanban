...
**`objective.md` is the SINGLE SOURCE OF TRUTH for monitor policy** — including its **TUNABLE TARGETS** block (`ACTIVE_AGENTS_TARGET`, `BACKLOG_FLOOR`, `MAX_NEW_STARTS_PER_CYCLE`). Both *objective-reading* mechanisms read this one file: the out-of-process loop, and the in-process **Monitor Butler** whose `STRATEGY_FILE` points at it. (The deterministic `runMonitorCycle` variant does **not** read it — it's driven by prefs.) There is intentionally **no** `.claude/monitor-strategy.md` — don't recreate one; edit `objective.md` to steer either mechanism. `loop.sh` **re-reads `objective.md` at the start of every iteration**, so policy/target edits (e.g. the WIP target `ACTIVE_AGENTS_TARGET`) take effect on the next cycle with **no restart** — only changes to `loop.sh` itself (or its env knobs `MONITOR_AGENT` / `MONITOR_SLEEP` / `MONITOR_ITER_TIMEOUT`) need a loop restart.
