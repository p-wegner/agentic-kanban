...
### Two kinds of monitor: out-of-process loop vs in-process monitor
There are **two fundamentally different ways** to drive the board, and they must not be confused:

| | **Out-of-process loop** (`scripts/board-monitor/`) | **In-process monitor** (inside the server) |
|---|---|---|
| What it is | A cron-like shell driver (`loop.sh`) that spawns a **fresh, short-lived agent session** every cycle, each reading `objective.md`, then exits | Code running **inside the live server process** that wakes on a `setTimeout` and acts on the DB directly |
| Harness | A full agent CLI — **Claude Code** (`claude --print`, default) or **codex** (`codex exec`), selected by `MONITOR_AGENT` | No separate agent process |
| Two variants | — (single mechanism) | **(a) `runMonitorCycle`** — *deterministic* code (relaunch/merge/nudge/auto-start), gated by the `auto_monitor` pref + `auto_monitor_interval`. **(b) Monitor Butler** (`monitor-butler.ts`, `monitor_butler_enabled`, off) — an *LLM* in-process session that reads `objective.md`. |
| Durable memory | Outside the model: board + git + a bounded, gitignored rolling `state.md` (last ~40 cycles) → every run disposable & crash-resilient | Process memory + DB; dies/restarts with the server |
| Crash behavior | Survives a server restart; an agent crash only loses that one cycle | A server crash takes the monitor with it; `auto_monitor` is force-disabled on every boot (`startup-tasks.ts`) |
| How to start/stop | `nohup bash scripts/board-monitor/loop.sh > /dev/null 2>&1 & disown` / `touch scripts/board-monitor/STOP` (or kill `loop.pid`) | Toggle the `auto_monitor` / `monitor_butler_enabled` prefs (Settings → Workflow → Board Monitoring) |
| Used here? | **YES — this is our dogfooding control plane for THIS dev board.** | The `runMonitorCycle` variant is the **shipped app default for other projects**. Both in-process variants are **off by default on this board.** |

The autonomous control plane that keeps **this** board moving (server alive, merge finished work, unstick stale sessions, refill backlog) is the **out-of-process loop** `scripts/board-monitor/` — a fresh short-lived agent session every ~30 min by default (`MONITOR_SLEEP=1800`; `loop.sh` + `objective.md`), running **Claude Code** unless `MONITOR_AGENT=codex`. It is **not** one long-running agent and **not** the in-process monitor. Rationale and the measured A/B/C tradeoff: `docs/decisions/006-board-monitor-orchestrator-architecture.md`. The `board-monitor` **skill** (`.claude/skills/board-monitor/`) is the system-level health checklist a run invokes — the loop is the harness.

**`objective.md` is the SINGLE SOURCE OF TRUTH for monitor policy** — including its **TUNABLE TARGETS** block (`ACTIVE_AGENTS_TARGET`, `BACKLOG_FLOOR`, `MAX_NEW_STARTS_PER_CYCLE`, `REFILL_FOCUS`). Both *objective-reading* mechanisms read this one file: the out-of-process loop, and the in-process **Monitor Butler** whose `STRATEGY_FILE` points at it. There is intentionally **no** `.claude/monitor-strategy.md` — don't recreate one; edit `objective.md` to steer either mechanism. `loop.sh` **re-reads `objective.md` at the start of every iteration**, so policy/target edits (e.g. the WIP target `ACTIVE_AGENTS_TARGET`) take effect on the next cycle with **no restart** — only changes to `loop.sh` itself (or its env knobs `MONITOR_AGENT` / `MONITOR_SLEEP` / `MONITOR_ITER_TIMEOUT`) need a loop restart.

### The strategic targets reach all three mechanisms — via two channels
The **Strategy Bullseye** UI (`StrategyTargetsView.tsx`, saved as the `board_strategy_<projectId>` preference) is the human-facing source for the targets. It feeds all three monitors through **two** channels that it keeps in sync:

1. **Generated `objective.md` block** (for the two *agent-driven* mechanisms — out-of-process loop + Monitor Butler). On save, `preference.service.ts → writeStrategyObjective()` regenerates the `<!-- STRATEGY_BULLSEYE_GENERATED_START … END -->` region in `objective.md`. The agents read it as prose.
2. **Direct pref read** (for the *deterministic* `runMonitorCycle` — the shipped app default). `runAutoStart` / `runBacklogEmptyStrategy` call `resolveMonitorTunables(prefMap, projectId)` (in `strategy-objective.service.ts`), which derives `{activeAgentsTarget, backlogFloor, maxNewStartsPerCycle, refillFocus}` from the same `board_strategy_<projectId>` pref. **Mapping:** `activeAgentsTarget` → WIP target (was `nudge_wip_limit`); `maxNewStartsPerCycle` → per-cycle launch cap counted across both auto-start passes; `backlogFloor` → the backlog-refill trigger threshold (was hardcoded "empty"); `refillFocus` → bug-only vs balanced generation prompt.

> **Backward-compat:** when a project has **no** `board_strategy` pref, `resolveMonitorTunables` falls back to the legacy `nudge_*` prefs (WIP = `nudge_wip_limit`, floor = 1 so refill only fires at truly-empty, no per-cycle cap). So untouched projects behave exactly as before; the Bullseye is an opt-in override.

> **Caveat — current dev board:** this board's `objective.md` TUNABLE TARGETS are currently **hand-authored** (no generated markers). Saving the Bullseye would overwrite that region with the UI values. If you hand-tune `objective.md` here, know that a Bullseye save will clobber it. Edit one or the other deliberately.
