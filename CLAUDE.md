...
- **Conflict detection uses `git merge-tree`**: `detectConflicts()` in both git services uses `git merge-tree --write-tree --no-messages HEAD <baseBranch>` (read-only, no working-tree changes). Exit 0 = clean merge, exit 1 = conflicts. Stdout: line 1 is the merged-tree SHA; subsequent lines are staged-entry records `<mode> <sha> <stage>\t<file>` where stage 1/2/3 marks conflicting files. Parse unique filenames from those lines. Never use `merge --no-commit --no-ff` for conflict detection — it mutates the working tree, `merge --abort` throws when there is no MERGE_HEAD ("Already up to date" case), and concurrent board requests can race on the same worktree.
- **Shared package must be rebuilt after schema changes**: `@agentic-kanban/shared` is consumed via its compiled `dist/` output (the `import` condition in package exports resolves to `./dist/schema/index.js`). `tsx watch` hot-reloads server source but does NOT watch or rebuild the shared package. After adding columns to `packages/shared/src/schema/*.ts`, always run `pnpm --filter @agentic-kanban/shared build` before restarting the server — otherwise Drizzle throws `TypeError: Cannot convert undefined or null to object` when building queries that reference the new fields.
