...
- **Codex hook parity**: `.codex/hooks.json` must route Codex `PreToolUse` shell checks through `.claude/hooks/smart-hooks-runner.js` so Claude and Codex share the same safety policy. When adding a new Claude safety hook, make it work with Codex hook input too (`tool_name`, `tool_input.command`, `cwd`) instead of duplicating logic.
...
- **Agent env separates board vs worktree ports**: In agent sessions, `KANBAN_SERVER_PORT` stays pointed at the main board API. Use `KANBAN_WORKTREE_SERVER_PORT` / `KANBAN_WORKTREE_CLIENT_PORT` for worktree dev-server cleanup and local visual checks; `PORT`, `SERVER_PORT`, and `VITE_PORT` are set to those worktree ports for subprocesses.
