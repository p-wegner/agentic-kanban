...
- **Hook paths on Windows**: Use absolute paths for hook commands in `settings.json`. Relative paths like `.claude/hooks/...` fail when Claude Code's CWD shifts (e.g., when working in a package subdirectory). `$CLAUDE_PROJECT_DIR` is not expanded in hook command strings. Example: `"node C:\\andrena\\agentic-kanban\\.claude\\hooks\\smart-hooks-runner.js Stop"`
