...
**Only fall back to REST API** when an operation has no MCP tool or CLI equivalent. All common workspace actions now have CLI and MCP equivalents (including launch and review). REST is a last resort if the server is not reachable.
