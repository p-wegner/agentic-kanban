...
- **`node_modules` is now junction-linked into new worktrees** (Dependency Symlinks is ON for this project: root + every `packages/*/node_modules` from the main checkout). So `pnpm test:mine` / `pnpm exec vitest` / `tsc` **run IN the worktree** — no more "relocate to main" dance, no `pnpm install`. Caveats: (1) worktrees created *before* this was enabled have no junctions — relocate to main or `pnpm install` once; (2) **never run `pnpm install`/`add` in a junctioned worktree to "fix" things** — it would write through the junction into main. The `validate-command-safety` hook intercepts this: a real dep change (`pnpm add <pkg>`, or drifted manifests) auto-isolates the worktree first; an unnecessary reinstall is blocked. To change a branch's deps, use `pnpm add <pkg>` (auto-isolates).
