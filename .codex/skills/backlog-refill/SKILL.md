...
## Rules
1. **Real, specific, scoped tickets — sized for the GATE, not just the agent (#661).** Title + description that an agent could pick up and implement in one focused workspace. No vague "improve X" — concrete acceptance criteria, following `CLAUDE.md` scope-guard. Bigger features are encouraged, but each individual ticket must still be implementable in one workspace (split ambitious work into linked tickets instead of writing one unimplementable mega-ticket). The floor matters as much as the ceiling: every ticket costs a full worktree + review + ~40-min merge-gate run, so a change an agent finishes in a few minutes (one helper, one rename, one doc tweak) is TOO GRANULAR as its own ticket — fold it into the adjacent ticket it belongs with, or emit the slices as one batch linked `coupled_with` (rule 4) so the board runs them as one ticket group.
...
4. **Declare coupling at creation (#918).** When a single feature genuinely splits into vertical slices that touch the SAME code (e.g. a UI panel + the endpoint that feeds it, or two tickets editing one central file), DON'T emit them as independent tickets that will start as conflicting parallel workspaces. Create them in ONE `mcp__agentic-kanban__create_issues_batch` call and add a `coupled_with` edge between them in the same call's `dependencies` (by 0-based index, `type: "coupled_with"`). This captures the coupling at birth — the monitor then starts the whole component as ONE ticket-group workspace (#661: one agent, one review, one merge-gate run; each ticket keeps its identity and closes with the group), and the gated auto-contract step can alternatively collapse them. Only declare `coupled_with` when you KNOWINGLY produce coupled slices; independent tickets stay independent. (Sequential ordering is still `depends_on`, not `coupled_with`.) For a backlog that ALREADY contains granular adjacent tickets, `mcp__agentic-kanban__propose_ticket_groups` runs the consolidation scan (preview first, `apply: true` to write the edges).
5. **Tag them.** Add a `backlog-refill` tag (create it once via `create_tag` if missing) so auto-generated tickets are traceable and you can audit the mix later. With `create_issues_batch`, pass `tags: ["backlog-refill"]` per issue.
6. **Stay in scope.** You are *generating work*, not doing it. Create the tickets and stop; the monitor's normal loop pulls and starts them through board workspaces.
