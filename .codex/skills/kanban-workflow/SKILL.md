...
1. **Run tests** — for refactoring tasks, run only the tests relevant to changed files (vitest v4 uses `related` as a subcommand, not a flag):
   ```
   pnpm --filter agentic-kanban exec vitest related <changed-files>
   ```
...
10. **Targeted tests for refactoring** — use `vitest related <changed-files>` (vitest v4 subcommand) instead of the full suite when refactoring. It's faster and proves the changed code is covered without re-running unrelated tests. ⚠️ `--related` flag is removed in vitest v4 — use `pnpm --filter agentic-kanban exec vitest related <files>` instead.
