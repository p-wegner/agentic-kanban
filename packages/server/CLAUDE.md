...
Settings use a single server-side whitelist: `SETTINGS_KEYS` in `packages/server/src/services/preference.service.ts` (both `getSettings` and `updateSettings` filter against it; the `PUT /api/preferences/settings` route just delegates to `updateSettings`). A key NOT in `SETTINGS_KEYS` (and not matching `isAllowedDynamicKey`) is now REJECTED LOUDLY on write: `updateSettings` returns `{ applied, dropped }`, the valid keys are still persisted, and the route returns **422** with `{ ok: false, applied, droppedKeys, error }` listing the un-whitelisted keys (#874). This used to be a SILENT drop — the toggle appeared to work but never persisted (it bit `auto_rebase_on_continue` and `skip_preflight`). So a new setting still requires: add the key to `SETTINGS_KEYS`, and add it to the `Settings` interface + `DEFAULT_SETTINGS` in `SettingsPanel.shared.tsx` — but now a forgotten `SETTINGS_KEYS` entry surfaces as a 422 instead of a confusing no-op. Per-project dynamic keys go through an `isAllowedDynamicKey` regex instead. The config import route (`config-export-import.ts`) calls `updateSettings` directly and surfaces `droppedKeys` as a NON-fatal field (an imported config may carry keys this build doesn't know). (The old advice about a `keys`/`allowedKeys` array in `routes/preferences.ts` is obsolete — that route no longer holds a whitelist.)
