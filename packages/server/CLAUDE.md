...
## Issue numbers
Auto-incrementing per project via `MAX(issue_number) + 1` in `POST /api/issues`. `issue_number` added in migration 0006. The `MIGRATION_FILES` array in `src/__tests__/helpers/migrations.ts` must include every migration SQL file — missing entries cause `SQLITE_ERROR: no such column` in unit tests.
