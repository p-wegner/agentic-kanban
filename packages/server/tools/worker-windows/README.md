...
| `ak-worker-tray.ps1` | WinForms `NotifyIcon`. grey = not installed/stopped, red = daemon down or disconnected, yellow = connected but the board is unreachable from here, green = connected and idle, blue = running N sessions. State comes from the log tail plus a process check; the board `/api/health` probe runs on a slower timer. Single-instance via a named mutex. Two extra red states rank **above** connection and sessions, because those are read from a log an orphan is no longer writing: `daemon ORPHANED - no supervisor` and `log frozen - state unknown`. |
...
| `ak-worker-process-match.test.ps1` | Regression test for the process filter. The old "command line contains *agentic-kanban* and *worker*" test matched the tray and the dashboard too — they are themselves files under this directory — so `-Stop` killed whichever enumerated first and printed `stopped`. Matching is now on `--board`, which only the daemon's argv carries. |
