...
export async function startServer(port?: number) {
      ...
      if (reviewSessionIds.has(sessionId)) {
        ...
        if (prefMap.get("learning_step_after_review") === "true" && workspace.workingDir) {
          try {
            ...
            const learnSessId = await sessionManager.startSession(workspace.id, learningPrompt, agentCmdLearn, agentArgsLearn ? agentArgsLearn.split(" ") : undefined, undefined, claudeProfileLearn, undefined, undefined, undefined, undefined, undefined, "learning", profileSelectionLearn);
      ...
      if (hasCommittedChanges) {
        ...
        if (autoReview) {
          ...
          try {
            ...
            const reviewSessionId = await sessionManager.startSession(workspaceId, reviewPromptText, agentCommand, reviewArgsWithModel, undefined, claudeProfile, undefined, undefined, undefined, undefined, provider, "review", profileSelection);
    ...
    try {
      ...
      if (prefMapLearning.get("learning_step_before_merge") === "true" && workspace.workingDir) {
        try {
          ...
          const profileSelectionLearn = claudeProfile ? { provider: "claude" as const, name: claudeProfile } : undefined;
          const learningSessId = await sessionManager.startSession(workspace.id, learningPrompt, agentCmd, agentArgs ? agentArgs.split(" ") : undefined, undefined, claudeProfile, undefined, undefined, undefined, undefined, undefined, "learning", profileSelectionLearn);
  ...
  app.post("/api/workspaces/:id/review", async (c) => {
    ...
    try {
      ...
      const reviewSessionId = await sessionManager.startSession(workspaceId, reviewPromptText, agentCommand, reviewArgsWithModel, undefined, claudeProfile, undefined, undefined, undefined, undefined, provider, "review", manualProfileSelection);
      reviewSessionIds.add(reviewSessionId);
