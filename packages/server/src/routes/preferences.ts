...
export function createPreferencesRoute(database: Database = db) {
  ...
  router.get("/settings", async (c) => {
    const keys = ["agent_command", "agent_args", "output_parser", "mock_agent", "skip_permissions", "claude_profile", "permission_prompt_tool", "auto_review"];
    const rows = await database
      .select()
      .from(preferences);
  ...
  router.put("/settings", async (c) => {
    ...
    const now = new Date().toISOString();
    const allowedKeys = ["agent_command", "agent_args", "output_parser", "mock_agent", "skip_permissions", "claude_profile", "permission_prompt_tool", "auto_review"];
