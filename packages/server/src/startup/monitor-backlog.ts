 ...
 */
function buildGenerationPrompt(skillName: string, refillFocus: "bugfix-only" | "balanced" = "balanced"): string {
  const focusLines = refillFocus === "bugfix-only"
    ? [
        "REFILL_FOCUS = bugfix-only: create ONLY tickets for real, reproducible bugs",
        "(derived from merged diffs, `docs/learnings/`, server error logs, or failing/flaky",
        "tests). Do NOT create feature or enhancement tickets this run.",
      ]
    : [
        "REFILL_FOCUS = balanced: create a balanced mix of bug-fix and feature/enhancement",
        "tickets, weighted toward the highest-value work.",
      ];
  return [
    `The board backlog is below its floor. Run the \`${skillName}\` skill to discover and create`,
...
/**
 * When the backlog drops below its floor (count of unstarted Todo issues without an
 * open workspace `< BACKLOG_FLOOR`), optionally trigger an agent skill that generates
 * new tickets.
 *
...
export async function runBacklogEmptyStrategy(
  ...
  for (const inProgressSt of inProgressStatuses) {
    ...
    const hostIssueId = await createHostIssue({
      ...
      description: `Auto-generated by the board monitor's backlog-refill strategy. Runs the \`${skillName}\` skill to create new local-only, high-value tickets when the backlog drops below its floor (REFILL_FOCUS = ${tunables.refillFocus}).`,
    }, nowIso);
    ...
    const resp = await fetch(`${baseUrl}/api/workspaces`, {
      ...
      body: JSON.stringify({
        ...
        branch,
        skillName,
        customPrompt: buildGenerationPrompt(skillName, tunables.refillFocus),
