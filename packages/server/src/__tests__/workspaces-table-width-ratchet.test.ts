 ...
 *
 * `workspaces` has 62 columns (88 before #781 extracted the first family; #798 took it from
 * 81 to 77 to 75 to 67, and #815 to 62). The next widest
 * table in the schema has 23 (`issues`, `repos`) and the median across 44 tables is 9, so this
 * is not "a wide table" — it is six remaining separate concerns flattened into one row by
 ...
 * `code_metrics_` and `latest_symlink_`; #815 lowered 67 → 62 and dropped `merge_gate_`. That is exactly the shape a successful extraction takes.
 */
 ...
 * (8) and `merge_gate_` (5) are deliberately ABSENT, not zeroed: #781 extracted the first to
 * `workspace_merge_backoff`, #798 the next three to `workspace_review_preflight`,
 * `workspace_code_metrics` and `workspace_symlink_run`, and #815 `merge_gate_` to
 * `workspace_merge_gate`. There is no such prefix on this table
 ...
 * at ~7 files and is 4): `conflict_cache_` → `latest_setup_` → `summary_` →
