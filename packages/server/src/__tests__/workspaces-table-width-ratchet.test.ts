 ...
 *
 * `workspaces` has 46 columns (88 before #781 extracted the first family; #798 took it from
 * 81 to 77 to 75 to 67, and #815 to 62 to 59 to 51 to 46). The next widest
 * table in the schema has 23 (`issues`, `repos`) and the median across 44 tables is 9, so this
 * is not "a wide table" — it is three remaining separate concerns flattened into one row by
 ...
 * `code_metrics_` and `latest_symlink_`; #815 lowered 67 → 62 → 59 → 51 → 46 and dropped
 * `merge_gate_`, `conflict_cache_`, `latest_setup_` and `summary_`. That is exactly the shape
 * a successful extraction takes.
 */
 ...
 * (8), `merge_gate_` (5), `conflict_cache_` (3), `latest_setup_` (8) and `summary_` (5) are
 ...
 * `workspace_symlink_run`, and #815 the last four to `workspace_merge_gate`,
 * `workspace_conflict_cache`, `workspace_setup_run` and `workspace_summary`. There is no such
 ...
 * `summary_` was the one published count that held, at 5 naming the column):
 * `diff_stat_cache_` → `scorecard_`. `fork_` and `showdown_` are permanently out
