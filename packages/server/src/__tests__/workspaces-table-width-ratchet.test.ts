 ...
 *
 * `workspaces` has 41 columns (88 before #781 extracted the first family; #798 took it from
 * 81 to 77 to 75 to 67, and #815 to 62 to 59 to 51 to 46 to 41). The next widest
 * table in the schema has 23 (`issues`, `repos`) and the median across 44 tables is 9, so this
 * is not "a wide table" — it is two remaining separate concerns flattened into one row by
 ...
 * `code_metrics_` and `latest_symlink_`; #815 lowered 67 → 62 → 59 → 51 → 46 → 41 and dropped
 * `merge_gate_`, `conflict_cache_`, `latest_setup_`, `summary_` and `diff_stat_cache_`. That
 * is exactly the shape a successful extraction takes.
 */
 ...
 * (8), `merge_gate_` (5), `conflict_cache_` (3), `latest_setup_` (8), `summary_` (5) and
 * `diff_stat_cache_` (5) are
 ...
 * `workspace_symlink_run`, and #815 the last five to `workspace_merge_gate`,
 * `workspace_conflict_cache`, `workspace_setup_run`, `workspace_summary` and
 * `workspace_diff_stat_cache`. There is no such
 * prefix on this table any more, and an entry of 0 would be a claim about a family that no
 * longer exists here.
 * Remaining order, cheapest coupling first, with the counts RE-DERIVED per family at cut time
 * (#798 and #815 both found the published estimates prose-inflated — `merge_gate_` was listed
 * at ~7 files and is 4, `conflict_cache_` at 11 and is 5, `latest_setup_` at 10 and is 8;
 * `summary_` was the one published count that held, at 5 naming the column, and
 * `diff_stat_cache_` was published at 10 and is 13 — the first count that was UNDER-stated,
 * because `workspace-diff.service.ts` and two test seeders read the columns off a
 * `select()`-everything row that no grep on the table name can see).
 * One family remains: `scorecard_`. `fork_` and `showdown_` are permanently out
