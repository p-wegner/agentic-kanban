...
6. Harness classification (#1021): if the ticket is work on the MACHINERY rather than the product — the pre-merge gate, guards, ratchets, always-run markers, the impact map / test selection, typecheck, hooks, the merge path, the monitor cycle — give it the \`harness\` tag (\`POST /api/issues/:id/tags\` with the tag's id, creating the tag via \`POST /api/tags\` if the board has none yet). The monitor budgets how many concurrent builders may run harness tickets (Strategy Bullseye → Harness %), so a missing tag understates that spend and a wrong one costs a deferred cycle. It is a proposal, not a verdict: leave an existing tag alone if a human set it, and never tag a ticket that changes what a USER sees.
