...
4. Report the new template id and how to use it (pick it on issue create, or set isDefault for a ticket type).`,
  ...
  },
  {
    ...
    prompt: `You are setting up the machine you are running on as a **fleet worker** for a remote agentic-kanban board. When it is connected, the board can schedule ticket work onto this machine instead of running every agent on the board host.
