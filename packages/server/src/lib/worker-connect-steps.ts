/**
 * The worker-connect runbook, as data (#774, moved out of `cli/commands/worker.ts` by #1089).
 *
 * Exported from here — not from the CLI command module — so BOTH the CLI (`worker
 * instructions`) and `GET /api/workers/connect-info` (the Runners view's Connect tab) render
 * the SAME steps instead of two copies that drift. A route may not import `cli/` (that module
 * pulls in Commander and process-exit side effects meant for a terminal), so the steps live in
 * `lib/` where either side can reach them.
 */
 ...
 * `fleet-worker` agent skill, the docs, and the Connect tab all render the SAME steps
