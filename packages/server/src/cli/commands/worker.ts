...
// `@agentic-kanban/shared/lib/*` specifiers.
import type { WorkerConnectStep } from "@agentic-kanban/shared/lib/worker-connect-steps";
// The `buildWorkerConnectSteps` FUNCTION lives in `server/src/lib/` (arch loop, ak-1094): a
// `server-route` (`routes/workers.ts`, `GET /api/workers/connect-info`) may not import
// `server-cli` per the pattern-language spec, so the function moved to pure server-lib and
// this CLI module re-imports it instead of defining it, keeping `worker instructions` and the
// Connect tab's steps byte-identical.
import { buildWorkerConnectSteps } from "../../lib/worker-connect-steps.js";
// Re-exported for existing importers (e.g. worker-connect-instructions.test.ts) that reach
// this function through this module rather than the new lib path directly.
export { buildWorkerConnectSteps };
