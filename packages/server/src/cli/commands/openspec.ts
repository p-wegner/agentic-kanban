...
Examples:
  ...
  $ pnpm cli -- openspec list --project <project-id>`,
    ...
    .action(cliAction(async (options: { project?: string; json?: boolean }) => {
...
Examples:
  ...
  $ pnpm cli -- openspec show butler-context --project <project-id>`,
    ...
    .action(cliAction(async (domain: string, options: { project?: string; json?: boolean }) => {
      const projectId = options.project ?? (await getActiveProjectId());
      const repoPath = await resolveRepoPath(projectId);
      if (!repoPath) {
        console.error(`Project '${projectId}' not found.`);
...
Examples:
  ...
  $ pnpm cli -- openspec validate --json`,
    ...
    .action(cliAction(async (change: string | undefined, options: { project?: string; json?: boolean }) => {
      const projectId = options.project ?? (await getActiveProjectId());
      const repoPath = await resolveRepoPath(projectId);
      if (!repoPath) {
        console.error(`Project '${projectId}' not found.`);
