...
/**
 * The board's declared Node floor (root CLAUDE.md: "Node LTS 22 floor (#731)"). Hard-coded
...
export function checkNodeVersion(nodeVersion: string = process.version): DoctorCheck {
  ...
  if (major < MIN_SUPPORTED_NODE_MAJOR) {
    return bad(
      ...
      `this machine runs Node ${nodeVersion}, older than the board's declared floor (>=${MIN_SUPPORTED_NODE_MAJOR})`,
      `Install Node ${MIN_SUPPORTED_NODE_MAJOR} LTS or newer, then reinstall the worker package under it.`,
    );
  }
  return ok("node runtime version", `Node ${nodeVersion} (floor is >=${MIN_SUPPORTED_NODE_MAJOR})`);
}
