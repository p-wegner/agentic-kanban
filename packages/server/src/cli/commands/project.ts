...
`)
    .action(cliAction(async (nameOrId: string) => {
...
`)
    .action(cliAction(async (nameOrId: string | undefined, opts: { out?: string }) => {
...
`)
    .action(cliAction(async (file: string, nameOrId: string | undefined) => {
      const project = await resolveProject(nameOrId);
      if (!project) {
        console.error(`Project "${nameOrId ?? "(active)"}" not found.`);
        process.exit(1);
