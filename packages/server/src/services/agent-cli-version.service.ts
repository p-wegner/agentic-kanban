...
export interface CliVersionConfig {
  ...
  minSupported: string;
  /**
   * First version known (or assumed) to break the flag contract (exclusive). When a
   * detected version is >= this, preflight emits a warning, not a hard error — the
   * CLI may still work, but the contract is no longer guaranteed.
   */
 ...
 * describes), not to pin a patch version. Bump these when a provider's flag
 * contract is verified against a newer release.
 */
export const CLI_VERSION_CONFIG: Record<ProviderName, CliVersionConfig> = {
  claude: { versionArgs: ["--version"], minSupported: "1.0.0", maxKnown: null },
  codex: { versionArgs: ["--version"], minSupported: "0.20.0", maxKnown: null },
  copilot: { versionArgs: ["--version"], minSupported: "0.1.0", maxKnown: null },
  ...
  pi: { versionArgs: ["--version"], minSupported: "0.70.0", maxKnown: null },
};
...
export async function detectCliVersion(
  ...
  if (config.maxKnown && compareSemver(version, config.maxKnown) >= 0) {
    return {
      ...
      version,
      status: "above-known",
      message: `${provider} CLI version ${version} is newer than the last verified version (< ${config.maxKnown}). Launch flags may have changed; verify before relying on it.`,
    };
  }
