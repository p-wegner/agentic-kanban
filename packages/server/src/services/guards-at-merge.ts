 ...
 *
 * Under `iterate` (and the coming `flow`, #1240) those guards are exactly the kind of check that
 * can run once per release-candidate sweep instead of once per merge: a guard that fails on the
 * base is a heal ticket (decision 019), not a lost branch. So the level derives the mode —
 * `iterate` -> `intersecting`, every other level -> `all` — and `standard`/`strict`/`fast`/
 * `sprint` are byte-for-byte untouched: `all` emits no env var and changes no message.
 *
...
export function guardsAtMergeForPosture(posture: Pick<RiskPosture, "level">): GuardsAtMerge {
  return posture.level === "iterate" ? "intersecting" : "all";
}
 ...
 * Empty for `all` so a non-`iterate` project's env is byte-identical to before #1232; the runner's
