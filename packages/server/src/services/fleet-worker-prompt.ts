...
5. **Verify:** \`agentic-kanban worker list --board <url>\` — this worker must read \`online\`. Then \`agentic-kanban worker doctor --board <url>\` for the whole chain from this side.
...
- **\`this workspace has not been trusted\` in the agent output** — expected on the FIRST dispatch to a new worker/project pair, and it recurs until someone acts on it. The worker clones each project into \`~/.agentic-kanban/worker/repos/<projectId>\`, a directory nobody has ever opened interactively, so Claude Code ignores that repo's \`.claude/settings*.json\` permission entries. The agent still runs — it is launched with permissions bypassed and the PreToolUse hooks fire regardless — so on an allow-only repo this is a confusing banner and nothing more; on a repo that defines \`deny\`/\`ask\` rules, those rules are what get dropped. \`agentic-kanban worker doctor\` names each untrusted checkout and which of the two cases it is. **Granting the trust is this machine's operator's call, not the board's** (the board holds no credentials here, by design): open Claude Code once in that directory and accept the dialog, or set \`projects["<that path, forward slashes>"].hasTrustDialogAccepted: true\` in \`~/.claude.json\`. The worker will never write that flag for you — a worker granting itself trust to code the board just pushed here is not a fix.
...
- **Clone/fetch fails or hangs** — the git-transport port is not reachable. It is OS-assigned per board boot unless the board pins it with \`KANBAN_GIT_HTTP_PORT\`; a cross-machine setup must pin it and open that port too. Note that \`worker doctor\`'s git-transport check reading \`????\` rather than PASS is NOT this failure: the board binds that listener on its first git dispatch and loses it on every restart, so "connection refused" is the normal state until the board has dispatched git work since it last started. A real clone failure is one you can see in a session, not one inferred from an idle probe.
