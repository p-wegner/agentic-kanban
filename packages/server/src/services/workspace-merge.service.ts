...
export function createWorkspaceMergeService(deps: {
  ...
  // #802: the rebase family (updateBase / abortRebase / rebaseRepo) lives in its own
  // cohesive sub-module — it moves branches onto their base and never lands anything,
  // unlike the merge half below. Bound here so every call site reads unchanged.
  const { updateBase, abortRebase, rebaseRepo } = createWorkspaceRebaseService({
    database,
    gitService,
    boardEvents,
    killWorktreeProcesses,
