 ...
 * Focus ranking (#606).
 *
 * Lifted verbatim out of `routes/focus.ts`, which was aggregating over three repositories
 * inline — one of the five routes doing so while `packages/server/CLAUDE.md` says the
 * adapter stays thin. The ranking is a pure projection over rows, so it belongs behind a
 * service the route calls.
 *
 * `now` is a required parameter here rather than defaulted: the route already accepted a
 * `?now=` override for deterministic tests, and making the service take it explicitly
 * means the point-in-time input is visible at every call site instead of hidden in a
 * default (#614).
 */
