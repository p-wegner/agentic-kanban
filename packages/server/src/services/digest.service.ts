 ...
 *
 * Lifted out of `routes/digest.ts`, the last of the five routes that aggregated over
 * repositories inline (four of them here) while `packages/server/CLAUDE.md` says the
 * adapter stays thin.
 *
 * `now` is a required parameter: the route already accepted a `?now=` override for
 * deterministic window tests, so taking it explicitly keeps the point-in-time input
 * visible at the call site rather than hidden in a default (#614).
 */
