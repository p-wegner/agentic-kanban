...
/**
 * `hook-posture.js` + `machine-capacity.js` — the runner's posture/capacity policy (#913).
 *
 * Unlike `git-topology-cache.js` above these are loaded DEFENSIVELY by the runner, so a
 * project that predates them keeps working; they are shipped anyway because a scaffolded
 * project that lacks them silently gets the one-size chain #913 exists to end (every Stop
 * paying a typecheck plus tests, and its generated stack rules spawning with no capacity
 * check at all). Written alongside the runner for the same reason as the topology cache:
 * a runner and its policy modules must not arrive in different versions.
 */
let _hookPostureSource: string | null | undefined = undefined;
function getHookPostureSource(): string | null {
  if (_hookPostureSource === undefined) _hookPostureSource = resolveHookSource("hook-posture.js");
  return _hookPostureSource;
}

let _machineCapacitySource: string | null | undefined = undefined;
function getMachineCapacitySource(): string | null {
  if (_machineCapacitySource === undefined) _machineCapacitySource = resolveHookSource("machine-capacity.js");
  return _machineCapacitySource;
}
...
export function ensureHookScaffold(repoPath: string, options: HookScaffoldOptions = {}): void {
  try {
    ...
    // --- hook-posture.js + machine-capacity.js (#913) ---
    // Also before the runner, for the same reason.
    writeBoardHookIfOutdated(join(hooksDir, "hook-posture.js"), getHookPostureSource());
    writeBoardHookIfOutdated(join(hooksDir, "machine-capacity.js"), getMachineCapacitySource());
