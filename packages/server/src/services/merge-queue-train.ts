...
import { createMergeTrain, findActiveMergeTrainForMembers, getMergeTrain, listActiveMergeTrainsForProject, updateMergeTrainState } from "../repositories/merge-train.repository.js";
...
async function beginMergeTrain(
  ...
  // #1158: reuse an existing `assembling`/`gating` row for the SAME member set instead of
  // minting a duplicate one when this attempt is racing a prior/concurrent attempt for the
  // exact same batch.
  ...
  // (`assembling`/`gating`) for a DIFFERENT member set. This is the seam every caller goes
