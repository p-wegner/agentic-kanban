...
/**
 * The row-quality classifiers moved to `./test-impact-outcome/row-quality.ts` when #997's
 * reasoning pushed this file past the 1000-line ceiling. Re-exported, not relocated behind a new
 * import path, because they are part of this module's public surface and their callers (the
 * recorder below, and the tests) should not have to know where the line count made them live.
 */
import {
  unattributedFailureReason,
  emptyChangeSetReason,
  unmeasuredUnionReason,
} from "./test-impact-outcome/row-quality.js";
// Imported AND re-exported: `recordGateOutcome` below calls all three, and a bare `export … from`
// re-exports without binding them in this module's scope.
