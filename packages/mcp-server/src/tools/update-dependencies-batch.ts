...
export async function applyUpdateDependenciesBatch(
  deps: ToolDeps,
  edges: DependencyBatchEdge[],
): Promise<
  | { ok: true; added: number; removed: number; skipped: { edge: DependencyBatchEdge; reason: string }[]; touchedProjectIds: string[] }
...
> {
  ...
  const skipped: { edge: DependencyBatchEdge; reason: string }[] = [];
  ...
  if (cycleError) {
    return { ok: false, message: (cycleError as { message: string }).message };
  }
  ...
  return { ok: true, added, removed, skipped, touchedProjectIds: [...touched] };
}
...
export function registerUpdateDependenciesBatch(server: McpServer, deps: ToolDeps = prodDeps) {
  server.tool(
    ...
    async ({ edges }) => {
      ...
      return { content: [{ type: "text" as const, text: JSON.stringify({ added: result.added, removed: result.removed, skipped: result.skipped }, null, 2) }] };
    },
  );
}
