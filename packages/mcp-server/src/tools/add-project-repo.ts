...
export function registerAddProjectRepo(server: McpServer) {
  server.tool(
    ...
    async ({ projectId, path, cloneUrl, name, setupScript, composeFile }) => {
      if (!path === !cloneUrl) {
        return { content: [{ type: "text" as const, text: "Error: provide exactly one of `path` or `cloneUrl`." }] };
      }
      try {
