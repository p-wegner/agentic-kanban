...
test.describe("Tags API", () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    // Create tag and issue upfront so all tests have them available
    const tagName = `test-tag-${Date.now().toString(36)}`;
    const tagRes = await request.post(`${BASE}/api/tags`, {
      data: { name: tagName, color: "#FF0000" },
    });
    createdTagId = (await tagRes.json()).id;

    const issueRes = await request.post(`${BASE}/api/issues`, {
      data: { title: `Tag test issue ${Date.now().toString(36)}`, statusId, projectId },
    });
    createdIssueId = (await issueRes.json()).id;
  });

  test.afterAll(async ({ request }) => {
    if (createdIssueId) {
      await request.delete(`${BASE}/api/issues/${createdIssueId}`);
    }
    if (createdTagId) {
      await request.delete(`${BASE}/api/tags/${createdTagId}`);
    }
  });
  ...
  test("POST /api/tags creates a tag", async ({ request }) => {
    ...
    const res = await request.get(`${BASE}/api/tags`);
    const body = await res.json();
    expect(body.some((t: { id: string }) => t.id === createdTagId)).toBeTruthy();
  });
  ...
  test("GET /api/tags includes created tag", async ({ request }) => {
    const res = await request.get(`${BASE}/api/tags`);
    const body = await res.json();
    expect(
  ...
  test("PATCH /api/tags/:id updates tag name and color", async ({
    ...
    const newName = `updated-tag-${Date.now().toString(36)}`;
  ...
  test("DELETE /api/tags/:id deletes tag", async ({ request }) => {
    // Create a separate tag so afterAll still has createdTagId to clean up if needed
    const ephemeralTagRes = await request.post(`${BASE}/api/tags`, {
      data: { name: `ephemeral-${Date.now().toString(36)}`, color: "#AABBCC" },
    });
    expect(ephemeralTagRes.status()).toBe(201);
    const { id: ephemeralTagId } = await ephemeralTagRes.json();

    const res = await request.delete(`${BASE}/api/tags/${ephemeralTagId}`);
    expect(res.ok()).toBeTruthy();
    const body = await res.json();
    expect(body.success).toBe(true);
    ...
    const list = await listRes.json();
    expect(
      list.some((t: { id: string }) => t.id === ephemeralTagId),
    ).toBeFalsy();
  });
});
