...
test.describe("Settings API", () => {
  test("GET /api/preferences/settings returns defaults", async ({ request }) => {
    const res = await request.get("http://localhost:3001/api/preferences/settings");
  ...
  test("PUT /api/preferences/settings saves settings", async ({ request }) => {
    const res = await request.put("http://localhost:3001/api/preferences/settings", {
      data: {
  ...
  test("PUT only persists allowed keys", async ({ request }) => {
    ...
    const getRes = await request.get("http://localhost:3001/api/preferences/settings");
  ...
  test("workspace launch uses agent_command from preferences", async ({ request }) => {
    ...
    await request.post(`http://localhost:3001/api/workspaces/${workspaceId}/setup`, {
      data: {},
    });
