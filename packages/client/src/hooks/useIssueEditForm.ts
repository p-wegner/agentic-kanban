 ...
 * the unsaved-changes flag, and the save / cancel / enhance handlers. Extracted
 * from the panel so the edit logic is one cohesive, separately-reasoned unit
 * (the panel destructures these with the same names, so its JSX and the
 * prop-sync / keydown effects are unchanged).
 *
 * setSaving is exposed because the panel's delete flow reuses the same busy flag;
 * the other busy/snapshot setters (the enhance state now owned by
...
export function useIssueEditForm(
  ...
  const fields: IssueEditFields = {
    title, description, issueType, dueDate,
