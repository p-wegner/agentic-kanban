    ...
    >
      ...
      {quickUpdate?.onTogglePinned && (
        <button
          ...
          className="flex w-full items-center gap-2 rounded px-2.5 py-1.5 text-left text-xs text-gray-700 hover:bg-gray-50 focus:bg-gray-50 focus:outline-none dark:text-gray-200 dark:hover:bg-gray-800 dark:focus:bg-gray-800"
        >
          <Icon className="h-3.5 w-3.5 shrink-0 text-amber-400" fill={issue.pinned ? "currentColor" : "none"} d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.562.562 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.562.562 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
      ...
      {showStartWorkspace && (
        <button
          ...
          className="flex w-full items-center gap-2 rounded px-2.5 py-1.5 text-left text-xs text-gray-700 hover:bg-gray-50 focus:bg-gray-50 focus:outline-none dark:text-gray-200 dark:hover:bg-gray-800 dark:focus:bg-gray-800"
        >
          <Icon className="h-3.5 w-3.5 shrink-0 text-brand-600" d="M13 10V3L4 14h7v7l9-11h-7z" />
      ...
      {showMoveToNext && (
        <button
          ...
          className="flex w-full items-center gap-2 rounded px-2.5 py-1.5 text-left text-xs text-gray-700 hover:bg-gray-50 focus:bg-gray-50 focus:outline-none dark:text-gray-200 dark:hover:bg-gray-800 dark:focus:bg-gray-800"
        >
          <Icon className="h-3.5 w-3.5 shrink-0 text-gray-400" strokeWidth={2.5} d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" />
      ...
      {quickUpdate?.onPriorityChange && (
        <div className="relative">
          <button
            ...
            className="flex w-full items-center gap-2 rounded px-2.5 py-1.5 text-left text-xs text-gray-700 hover:bg-gray-50 focus:bg-gray-50 focus:outline-none dark:text-gray-200 dark:hover:bg-gray-800 dark:focus:bg-gray-800"
          >
            <Icon className="h-3.5 w-3.5 shrink-0 text-gray-400" d="M3 7h18M3 12h12M3 17h6" />
            <span className="flex-1 truncate">Change priority</span>
            <Icon className="h-3 w-3 shrink-0 text-gray-400" d="M9 5l7 7-7 7" />
          </button>
          {prioritySubmenuOpen && (
            <div className="absolute left-full top-0 ml-1 w-36 rounded-md border border-gray-200 bg-white p-1 shadow-lg dark:border-gray-700 dark:bg-gray-900 z-10">
              {(["critical", "high", "medium", "low"] as const).map((p) => (
                ...
                >
                  ...
                  {issue.priority === p && <Icon className="ml-auto h-3 w-3 text-brand-600" strokeWidth={3} d="M5 13l4 4L19 7" />}
                </button>
              ))}
            </div>
          )}
      ...
      {allStatuses && allStatuses.length > 0 && (
        <div className="relative">
          <button
            ...
            className="flex w-full items-center gap-2 rounded px-2.5 py-1.5 text-left text-xs text-gray-700 hover:bg-gray-50 focus:bg-gray-50 focus:outline-none dark:text-gray-200 dark:hover:bg-gray-800 dark:focus:bg-gray-800"
          >
            <Icon className="h-3.5 w-3.5 shrink-0 text-gray-400" d="M8 7h12m0 0l-4-4m4 4l-4 4m-8 6H4m0 0l4 4m-4-4l4-4" />
            ...
            <div className="absolute left-full top-0 ml-1 w-40 rounded-md border border-gray-200 bg-white p-1 shadow-lg dark:border-gray-700 dark:bg-gray-900 z-10">
              {allStatuses.map((s) => (
                ...
                >
                  ...
                  {issue.statusName === s.name && <Icon className="ml-auto h-3 w-3 shrink-0 text-brand-600" strokeWidth={3} d="M5 13l4 4L19 7" />}
                </button>
              ))}
            </div>
          )}
