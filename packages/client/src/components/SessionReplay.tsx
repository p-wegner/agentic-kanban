...
function parseMessagesIntoTurns(
...
export function SessionReplay({
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    apiFetch<{ messages: AgentOutputMessage[] }>(`/api/sessions/${sessionId}/output`)
      .then((data) => {
        const parsed = parseMessagesIntoTurns(data.messages, outputFormat);
        setTurns(parsed);
        setCurrentTurnIdx(0);
