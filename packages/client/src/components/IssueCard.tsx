...
function IssueCardHeader({
  ...
  return (
    <div className="flex min-w-0 items-start justify-between gap-2">
      ...
      {isPendingWorkspace && (
        <Spinner className="w-3.5 h-3.5 shrink-0 text-brand-500 animate-spin mt-0.5" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
      )}
    </div>
  );
}
      ...
      <div className={`flex items-center gap-1.5 flex-wrap ${compact ? "mt-0.5" : "mt-1"}`}>
        ...
        {issue.isBlocked && (
          <Badge
            ...
            icon={<Icon
              ...
              viewBox="0 0 16 16"
              d="M8 1a2 2 0 0 1 2 2v4H6V3a2 2 0 0 1 2-2zm3 6V3a3 3 0 0 0-6 0v4a2 2 0 0 0 2 2v2.5a.5.5 0 0 0 1 0V9a2 2 0 0 0 2-2z"
            />}
          >
            blocked
          </Badge>
        )}
        {issue.awaitingManualStart && (
          <Badge
            tone="warning"
            title="In Progress with no agent on it, and this project's Start Mode is manual — nothing will pick it up until you start or resume it by hand"
            icon={
              <Icon className="w-3 h-3 shrink-0" d="M12 9v2m0 4h.01M5.07 19h13.86c1.54 0 2.5-1.67 1.73-3L13.73 4c-.77-1.33-2.69-1.33-3.46 0L3.34 16c-.77 1.33.19 3 1.73 3z" />
            }
          >
            no driver
          </Badge>
        ...
        {!issue.isBlocked && (issue as IssueWithStatus & { dependencyCount?: number }).dependencyCount ? (
          <Badge
            ...
            icon={<Icon className="w-3 h-3 shrink-0"><path d="M5 12h14M12 5l7 7-7 7"/></Icon>}
          >
        ...
        {issue.dueDate && (() => {
          ...
          return overdue ? (
            <Badge
              tone="danger"
              title={`Overdue: ${due.toLocaleDateString('en-US')}`}
              icon={
                <Icon className="w-3 h-3 shrink-0" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              }
            >
            ...
            <Badge
              tone="neutral"
              title={`Due: ${due.toLocaleDateString('en-US')}`}
              icon={
                <Icon className="w-3 h-3 shrink-0" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
              }
            >
  ...
  return (
    ...
    <div className={`flex flex-wrap items-center gap-1.5 transition-opacity ${compact ? "mt-1 flex sm:hidden sm:group-hover:flex" : "mt-1.5 opacity-100 sm:opacity-0 sm:group-hover:opacity-100"}`}>
      ...
      {showMoveToNext && (
        ...
        >
          <Icon className="w-3 h-3 shrink-0" strokeWidth={2.5} d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" />
          {nextStatusName}
        </button>
      )}
    </div>
