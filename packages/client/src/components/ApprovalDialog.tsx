...
export function ApprovalDialog({ requests, onResolve }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50">
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center gap-2">
          ...
          <h2 className="font-semibold text-gray-900 dark:text-gray-100">Permission Request</h2>
          {requests.length > 1 && (
            <span className="ml-auto text-xs text-gray-400 dark:text-gray-500">{requests.length} pending</span>
          )}
        </div>

        <div className="px-5 py-4 space-y-3">
          <p className="text-sm text-gray-700 dark:text-gray-300">
            The agent wants to use <span className="font-mono font-semibold text-gray-900 dark:text-gray-100 bg-gray-100 dark:bg-gray-800 px-1 rounded">{req.toolName}</span>
          </p>

          <div className="bg-gray-50 dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-700 p-3">
            <pre className="text-xs text-gray-700 dark:text-gray-300 whitespace-pre-wrap break-all max-h-48 overflow-y-auto font-mono">{toolInputPreview}</pre>
          </div>
        </div>

        <div className="px-5 py-3 border-t border-gray-200 dark:border-gray-700 flex flex-wrap gap-2 justify-end">
          <button
            onClick={() => handle("deny_session")}
            className="px-3 py-1.5 text-sm rounded border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
          >
            Deny session
          </button>
          <button
