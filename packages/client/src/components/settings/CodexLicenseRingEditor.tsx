...
function parseRing(raw: string | undefined): RingRow[] {
  ...
  try {
    ...
    return parsed.map((item) => {
      ...
      const mode: RingMode = codexHome ? "oauth" : "apikey";
      return {
...
export function CodexLicenseRingEditor({ settings, set }: { settings: Settings; set: SettingsTextSetter }) {
  ...
  return (
    <Field
      ...
      hint="Each row is a Codex account the board rotates through when one hits its usage limit. OAuth = a ChatGPT login in its own CODEX_HOME dir (run `$env:CODEX_HOME='<dir>'; codex login` once per account). API key = a config_<name>.toml in ~/.codex. The profile name also appears in the Agent Profile dropdown above."
    >
            ...
            {rows.map((row, i) => (
              <div key={i} className="flex flex-wrap items-center gap-2">
                ...
                <input
                  ...
                  placeholder={row.mode === "oauth" ? "C:\\Users\\you\\.codex-ki14" : "config_apikey1"}
                  className={`${inputClass} flex-1 min-w-[12rem] font-mono`}
                />
                <button
                  type="button"
                  onClick={() => commit(rows.filter((_, idx) => idx !== i))}
                  className="text-xs px-2 py-1.5 border border-gray-300 dark:border-gray-600 rounded text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800"
