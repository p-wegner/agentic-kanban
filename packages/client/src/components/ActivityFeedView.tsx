import type { ActivityEventType, ProjectActivityEvent, ProjectActivityResult } from "@agentic-kanban/shared";
// #704: moved to shared/src/types/api/. Re-exported so importers of this module are unchanged.
export type { ActivityEventType, ProjectActivityEvent, ProjectActivityResult };
...
const EVENT_ICONS: Record<ActivityEventType, React.ReactNode> = {
  issue_created: (
    <Icon className="h-3.5 w-3.5" d="M12 4v16m8-8H4" />
  ),
  status_changed: (
    <Icon className="h-3.5 w-3.5" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
  ),
  workspace_created: (
    <Icon className="h-3.5 w-3.5" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
  ),
  workspace_launched: (
  ...
  session_completed: (
    <Icon className="h-3.5 w-3.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
  ),
  session_failed: (
    <Icon className="h-3.5 w-3.5" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
  ),
  session_stopped: (
    <Icon className="h-3.5 w-3.5" d="M21 12a9 9 0 11-18 0 9 9 0 0118zM9 10a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1h-4a1 1 0 01-1-1v-4z" />
  ),
  comment: (
    <Icon
      className="h-3.5 w-3.5"
    ...
    <div className="flex flex-col h-full overflow-hidden p-4">
      ...
      {!loading && !error && data && data.events.length === 0 && (
        <div className="flex flex-col items-center justify-center flex-1 gap-2 text-center">
          <Icon className="h-8 w-8 text-gray-300 dark:text-gray-600" strokeWidth={1.5} d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
