...
export function CopyButton({ text }: { text: string }) {
  ...
  return (
    ...
    >
      {copied ? (
        <Icon className="h-3.5 w-3.5 text-green-500" d="M5 13l4 4L19 7" />
      ) : (
        <Icon className="h-3.5 w-3.5" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
      )}
      {copied && (
...
export function CopyLinkButton({ issueNumber }: { issueNumber: number }) {
  ...
  return (
    ...
    >
      {copied ? (
        <Icon className="h-4 w-4 text-green-500" d="M5 13l4 4L19 7" />
      ) : (
        <Icon
          className="h-4 w-4"
