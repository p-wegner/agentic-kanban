...
type Token = { t: "kw" | "str" | "num" | "cmt" | "fn" | "op" | "tag" | "attr" | "plain"; v: string };
...
function tokenColor(t: Token["t"]): React.CSSProperties {
...
function tokenizeMD(code: string): Token[] {
  // Headings
...
function highlightLine(code: string, lang: string): React.ReactNode {
              ...
              <Fragment key={pair.lineIdx}>
                ...
                >
                  ...
                  <td className={`px-2 ${pair.left?.type === "delete" ? "bg-red-50 text-red-800" : "text-gray-700 dark:text-gray-300"}`}>
                    {pair.left ? highlightLine(pair.left.content, lang) : ""}
                  </td>
                  ...
                  <td className={`px-2 ${pair.right?.type === "add" ? "bg-green-50 text-green-800" : "text-gray-700 dark:text-gray-300"}`}>
                    {pair.right ? highlightLine(pair.right.content, lang) : ""}
                  </td>
                </tr>
                {allComments.map((c) => (
                  <tr key={c.id}>
