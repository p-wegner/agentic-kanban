...
/**
 * The loop's home screen (#453).
 *
 * It used to be a wrapping row of pills joined by `—`: at 1440px the live 9-step pipeline
 * wrapped after step 5, so the connector logic broke across lines and the "pipeline" read as
 * two unrelated pill rows. It occupied 40px and answered almost nothing — no per-step cost
 * (though `byUnit` was already fetched a few hundred pixels below), no per-step artifacts
 * (only a `📄3` suffix you had to open a file to expand), and no way to tell locked from
 * pending.
 *
 * So: one row per step, which cannot wrap mid-connector at any width, with the state spelled
 * out, the version, the cost, and every artifact as its own chip.
 */
