...
const LINE_BASED_GUARDS: Record<string, string> = {
  ...
  "server/src/__tests__/temp-dir-namespace-guard.test.ts":
    "(2) the SITE scan is on the AST since #849 — the three passes match `mkdtemp(...)` / " +
    "`join(tmpdir(), ...)` as call SHAPES, so a wrapped call no longer slips past (there is a " +
    "bite test on exactly that two-line form). What still splits lines is the MARKER half, " +
    "and lines are its real unit: `findOkMarker` walks the contiguous comment block above a " +
    "site, comment adjacency is a line concept, and the orphan check needs each marker's own " +
    "LINE NUMBER to name it in the failure. #849 asked for this entry to be deleted outright; " +
    "it cannot be, because the same ticket asks the marker lookup to stay line-based — so it " +
    "is reclassified rather than removed, and the (3) HONESTLY EVADABLE disclosure is gone.",
