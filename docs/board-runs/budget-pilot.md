...
- **#783** — the #777 build-approval fix is INEFFECTIVE for from-scratch projects (runs at registration before the builder creates package.json/pnpm-workspace.yaml; never re-applies), the scaffold re-emits a bogus `allowBuilds:` placeholder, no `packageManager` pin so the toy runs under global pnpm 11.0.8 (which ignores the approval config), and the per-worktree verify gate FALSE-PASSES because the shared pnpm store already approved esbuild. Supersedes #777's Done claim.
