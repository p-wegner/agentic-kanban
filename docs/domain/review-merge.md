...
| Verify/smoke pre-merge quality gate (the enforcer that FAILS a merge) | `services/pre-merge-gate.service.ts:47` (`runPreMergeGate`); wired into `startup/exit-workflow.ts:536`, `startup/monitor-cycle.ts:247,323`, AND the manual/operator + orchestrator merge body `services/workspace-merge.service.ts` `doMerge` (#930 — runs after OpenSpec/migration prevalidation, before `executeWorkspaceMerge`; withholds the land on failure, always re-verifies, no readyForMerge skip) | service |
