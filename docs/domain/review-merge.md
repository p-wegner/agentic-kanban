...
- **`merge_strategy` triple meaning is easy to confuse.** `direct`/`monitor`/`merge_queue` (`merge-strategy.ts:4`) selects WHICH engine merges; `ReconcileStrategy` (`merge-queue.service.ts:70`) is a per-cluster HOW; and Start Mode (`manual`/`monitor`/`conductor`) is yet another axis. Three "mode" vocabularies overlap. *Inferred, unverified:* a misconfigured `merge_strategy=monitor` while the in-process monitor is off could leave reviewed work unmerged (the orchestrator only owns `merge_queue`).
