...
| **A Builder's writes are confined to its own worktree by a hard PreToolUse gate** — `prevent-cross-worktree-writes.js` blocks any write/edit whose target resolves into the main checkout or another worktree, and `require-read-before-write.js` blocks blind writes. This is the *runtime* enforcement of the "blast radius is contained" promise (the symlink guard above only covers setup). **Agent operating rule:** when it fires, STOP — it is a safety gate, not a bug; never weaken it or route around it (e.g. via an absolute path or a sibling worktree). Mirrors the DB-safety gate (see persistence-schema). | `.claude/hooks/prevent-cross-worktree-writes.js`, `.claude/hooks/require-read-before-write.js` (Codex parity via `.codex/hooks.json`) |
