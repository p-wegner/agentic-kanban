...
| **Provider-divergence write guard**: a `provider`/`*_profile` write that would put global prefs out of sync with the active project's Bullseye is rejected (422), nothing persisted | Makes the old passive "divergence banner" an enforced invariant so global prefs no longer drift from the Bullseye on any WRITE path — the guard now fires on HTTP/CLI/MCP/internal (all route through shared `setPreferenceChecked`; the #903 MCP side door is closed, b847bb84). Residual: the guard resolves the provider quota-free while launches are quota-aware, so a static pref can't perfectly mirror a time-varying selection (Ticket 12). Retires the `set-provider-default` skill's reason to exist (#903) | `preference.service.ts:110`, `shared/lib/checked-preference-write.ts`, `mcp-server/tools/set-preference.ts:99` |
...
| **Monitor tunables** (WIP/floor/starts/refill) | Bullseye fields (`strategy-objective.service.ts:64`); legacy `nudge_wip_limit` (read-only since #1102 — WIP is configured only in the Bullseye) | `resolveMonitorTunables` (`strategy-objective.service.ts:398`) — strategy wins, else legacy prefs | **YES — strategy vs legacy `nudge_*` fallback**; `source` field reports which drove a cycle |
