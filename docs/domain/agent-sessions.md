...
| **`session_messages` is durably PRUNED on a background schedule, separately from the in-memory buffer free.** A 6-hour interval (first run +30s) (a) deletes all `session_messages` for workspaces merged/closed > 3 days ago (`MERGED_WORKSPACE_RETENTION_DAYS`, batched 500/stmt) and (b) caps each still-active/overflow session at 2000 rows (`MAX_MESSAGES_PER_ACTIVE_SESSION`), deleting the oldest excess. | DB size must stay bounded as workspace history grows; for done workspaces the `.out` file on disk is the canonical record, so the DB rows are pure index and safe to drop. Distinct from the per-session in-memory buffer free (`ws-handler.ts:32`) — that reclaims RAM for one ended session; this reclaims persistent DB rows fleet-wide. | `session-message-pruner.service.ts:25` (`pruneOldSessionMessages`), `:57` (`capSessionMessages`), `:77` (`startSessionMessagePruner`); started from `server-start.ts:226` |
...
- **A non-silent ("chatty") runaway loop has no documented LIVE signal/threshold to justify a manual `/stop` — it burns tokens and holds a WIP slot invisibly.** The only inactivity-style catch in this module is the **launch-failure window** — and it fires solely on a fast (≤10s) exit with *zero* substantive output (`session-exit-state-machine.ts:25`,`:96`). There is no wall-clock or token-burn watchdog that trips while a session is *running*: the substantive-output flag is re-marked on every event (`broadcast.ts:120`), so an agent that keeps re-emitting while re-running the same failing tool never looks idle. Everything else is post-hoc and gated by the out-of-process monitor: (a) the repeated-failed-command recovery (`hasRepeatedFailedCommand`, ≥3 repeats, `monitor-cycle-rules.ts:7`) only runs inside `recoverStuckBuilder`, which is gated on **0 commits ahead** (`monitor-cycle.ts:146`) — a loop that has produced any commit slips past it; and (b) the `MAX_SESSIONS`-relaunch force-close (`= 10`, `monitor-cycle.ts:224`) only counts *sessions*, not within-session waste. The closest live signal — the monitor's >5-min nudge (`monitor-cycle.ts:389`) — is deliberately *suppressed* for exactly this case: `shouldSkipNudge` skips it when recent excerpts show the agent "actively working" (`:393`). So an operator's only live evidence of a chatty loop is the UI's live token/turn/tool stats; the system surfaces no threshold of its own to act on before the gated, post-hoc catches eventually fire. *(verified against the cited code.)*
