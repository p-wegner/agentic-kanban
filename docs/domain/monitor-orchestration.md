...
| **Post-merge dependency auto-chain is a THIRD auto-start path** (distinct from the cycle's `runAutoStart` fan-out and the backlog refill): when a blocker merges it selects exactly ONE Todo/Backlog issue whose `depends_on`/`blocked_by` are all resolved, respects the WIP cap, skips `no-auto-start`-tagged issues, excludes dependency-cycle participants, and RECORDS an audit comment on the issue (whether it starts one or skips, and why). Gated by the `dependency_auto_chain` pref. **REGRESSION-LOCK (incident):** it is gated by the project's Start Mode, NOT `dependency_auto_chain` alone — with Start Mode `manual` it is a clean no-op (not even an audit comment) even when `dependency_auto_chain=true`. | A merge event should pull the next ready dependent without a full cycle, but the cascade once had its own gate and kept auto-starting tickets with every "drive" switch off; routing it through `resolveStartPolicy().postMergeCascade` closes that leak. Shares `isResolvedDependencyStatusView` (the same landed-blocker readiness contract as `computeBlockerReadiness`). | `dependency-auto-chain.service.ts:216-217` (Start-Mode gate), `:90-184` (single-candidate selection: WIP `:107`, cycle `:132`, skip-tag `:137`), `:231-271` (audit comment); regression-lock `dependency-auto-chain.test.ts:235-259` |
| **The dependency-wave planner partitions candidates into ready / blocked / `cyclicInvalid`, and starts only the next ready wave that FITS under the WIP target.** Per-dependent it reports the specific un-landed upstream blockers; a fan-in dependent stays blocked until ALL its upstreams land. Wave-launched builders are forwarded `planMode:false` (#767) so they start in EXECUTE mode. | A planning surface for "start everything that's currently runnable" must never launch cycle members or over-fill WIP, and must show *which* upstream is holding each dependent. Wave builders skipping plan mode matches the New-Workspace execute default — a codex builder that plans first looks like a stalled session. Uses the SAME `computeBlockerReadiness` as the monitor auto-start path, so "landed-not-just-Done" stays one rule across both. | `dependency-wave.service.ts:166-226` (partition + per-dependent blockers via `computeBlockerReadiness` `:182`), `:234` (slice to `wip.available`), `:255-260` (`planMode:false`); tests `dependency-wave.service.test.ts:118-270` |
...
| Post-merge dependency auto-chain (THIRD auto-start path): single-candidate select + Start-Mode gate + audit comment | `services/dependency-auto-chain.service.ts` | domain logic |
| Dependency-wave planner (ready/blocked/cyclicInvalid partition, WIP-fit wave start, `planMode:false`) | `services/dependency-wave.service.ts` | domain logic |
