...
- **WIP has ONE meaning (#1102):** the per-column *visual* WIP limit (`wip_limit_<statusId>`, `wipLimits.ts`) was removed, so "WIP" now only ever means the monitor's agent target — the Strategy Bullseye's `activeAgentsTarget`, resolved by `resolveWipLimit` and shown on the toolbar's Autopilot chip. Stored `wip_limit_<statusId>` rows are left orphaned; nothing reads them.
