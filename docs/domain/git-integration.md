...
- **A stale on-disk `.git/index.lock` masquerades as a DB lock.** A crashed or killed git op (interrupted merge/rebase, a process taskkilled mid-write) can leave a `.git/index.lock` file behind. Git refuses ALL subsequent operations in that repo/worktree (`fatal: Unable to create '.../.git/index.lock': File exists`), so board actions that route through `git-exec` fail — and the surfaced symptom often looks like a hang or a generic "locked" error that is easily mistaken for a SQLite/`kanban.db` lock. **These are three distinct locks — don't conflate them:** (1) this on-disk `.git/index.lock` (a git artifact); (2) the **in-memory merge lock** (one merge at a time per repo, stale after 15 min — see review-merge.md, `MERGE_LOCK_STALE_MS`); (3) the **DB lock** on `kanban.db`. **Recovery for the index lock:** confirm no git process is actually in flight (no merge/rebase running), delete the stale `.git/index.lock`, then re-verify `git status` succeeds and the worktree isn't desynced (HEAD/working tree match the branch tip) before retrying the operation. **Never delete `kanban.db`** — a DB-lock symptom is a different problem (use the `db-doctor` skill), and the index-lock recovery must never touch the database.
