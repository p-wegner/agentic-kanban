...
| 2026-05-28 | Butler enhancements | Warm in-process Claude Agent SDK assistant per project (`butler-sdk.service.ts`), separate from the CLI-spawn task agents. This session added: per-project **model picker** (live switch via `query.setModel()`, no restart; pref `butler_model_<id>`) and **profile selector** (restarts session; pref `butler_profile_<id>`, falls back to global `claude_profile`); **slash-command autocomplete** (server merges SDK `supportedCommands()` + repo `.claude/skills`, deduped); **Stop button** (`POST .../interrupt` → `query.interrupt()`); accurate context-usage via `getContextUsage()` (not the cache-inflated turn-usage sum); SSE listeners moved to a project-keyed registry so streams survive clear/restart; markdown rendering via `@tailwindcss/typography` (+ inline-code pill); board orchestration via the one-step `POST /api/workspaces` launch (never `start_workspace`, never fabricate success); and a bundled user-facing **board-usage UI guide** (`butler/board-guide.ts`) the butler reads on demand via the `{{boardGuidePath}}` prompt placeholder. |
| 2026-06-20 | Server god-file decomposition (#853) | Behaviour-preserving decomposition of the four server god-files named in ticket #853, each its own commit. **(1)** `issue.service` cycle-detection DFS → pure `lib/dependency-graph.ts` (+13 tests), killing the duplicate-DFS drift vs `board-column.service` (985→916). **(2)** `project.service` `getBoard` projection (terminal-status override, staleness, column-cap) → pure generic `lib/board-view.ts` `buildBoardColumns` (+11 tests; 911→839). **(3)** `stack-profile` Gradle/KMP/Ktor detectors → `gradle-detect.service.ts` (+22 temp-repo tests; 1181→1043). **(4)** de-dup worktree port derivation into `worktree-ports.ts` helpers (+13 tests). **(5)** `workspace-crud.service.ts` (the 1608-line monster) fully split into four focused modules: a thin **crud delegator** (206), **`workspace-create.service`** (create orchestration + launch-preview, 666), **`workspace-provision.service`** (worktree setup + symlinks + setup script + agent config/prompt/skill + TDD hook + context, 376), and **`workspace-cleanup.service`** (stop/kill + worktree teardown + stale-worktree/cleanup-warning maintenance, 305). All extractions preserve the public surface + re-exported types (importers untouched). Verified: full server suite **2783 tests / 260 files green**, `tsc` 0, `lint:arch` 0. No server source file from #853 now exceeds ~700 lines; **+59 new unit tests**. |
| 2026-06-20 | Server arch quality pass (error mapping + CC) | Two cohesive architecture improvements on master, each its own commit. **(1) Transport-layer error handling drained to typed domain errors:** 10 of 11 route handlers that hand-rolled `try/catch` + `message.includes("not found")` string-matching now let services throw typed errors (`NotFoundError`/`ValidationError`), mapped in ONE place by the existing `domainErrorHandler`. Services migrated: `workspace-launch-failures`, `workspace-risk`, `file-contention`, `workspace-timeline`, `session-artifacts` (incl. `ENOENT`→404, path-traversal/bad-extension→400), `board-status` (`getBoardStatus`→`NotFoundError`). Routes `projects.ts` (×6) and `workspace-actions.ts` (×4) lost their try/catches; `voice-capture` kept its bespoke 422. Net −57 LOC from routes. **(2) `workspace-summary.service.ts` god-functions decomposed** via the pure-tested-lib seam pattern: `attachSessionData` CC 39→11 (`lib/workspace-summary-session.ts`: `selectLatestSessionsByWorkspace` w/ injected noise predicate, `parseContextTokensFromStats`; + extracted `collectLastToolAndMessages` I/O); `applyDiffStats` CC 29→5 (`lib/workspace-diff-cache.ts`: `selectCachedDiffStats`/`isPlanOnlySession`/`isDiffCacheStale`, `nowMs` injected). File max CC 39→23. +27 new unit tests. **(3) Two more pure-lib extractions:** `preference.service.isAllowedDynamicKey` CC 23→2 via a declarative prefix table (`lib/dynamic-preference-keys.ts`; adding a per-project setting is now a one-line edit instead of another regex; +8 tests), and `issue.repository.getIssueSummary` CC 32→13 by extracting the stats/duration projection (`lib/issue-summary-projection.ts`, typed, no `as any`; +9 tests). Verified: `tsc` 0, `lint:arch` 0, full server suite **264 files / 2827 tests green**. |
| 2026-06-20 | Server hotspot pass (repository + board-status projections) | Re-ran code-metrics, decomposed the top remaining server pure-CC functions via the pure-tested-lib seam. **(1)** `workspace.repository.getWorkspaceDetails` (the server's worst, CC 36) → the full row→DTO projection moved to `lib/workspace-details-projection.ts` (`mapWorkspaceDetailsRow` + `parseJsonArray`/`mapSymlinkRun`/`parseSessionContextAndTool` + private conflict/diff/setup mappers); repo fn now just 2 queries + a mapper call (CC 36→4); `WorkspaceDetails` re-exported so importers are unchanged; +12 tests. **(2)** `board-status.ts`: extracted `parseSessionStats` (CC 16), `selectLatestRelevantSession`, and `buildBoardStatusEntry` to `lib/board-status-entry.ts`; `getBoardStatus` CC 30→23 (remainder is inherent query/grouping/totals orchestration); +15 tests. Verified: `tsc` 0, `lint:arch` 0, full server suite **266 files / 2848 tests green**. |
| 2026-06-20 | Butler runLoop flattened (CC 38→9) | Took on the server's single worst remaining function, `butler-sdk.service.runLoop`, without touching the warm-SDK streaming/session mechanics. The five SDK message branches (init / stream_event / assistant / tool-results / result) moved verbatim into module-level handlers behind a flat `dispatchButlerMessage`; the catch cascade's precedence (abort > transient > resume-reset > fatal) extracted to a PURE tested lib `lib/butler-loop-classify.ts`, with the resume-reset recovery (drop dead resume id, restart fresh, re-send in-flight turn) isolated in `recoverButlerResume`. runLoop CC 38→9; handlers CC 4-7; +7 classifier unit tests. Verified: `tsc` 0, `lint:arch` 0, all 7 butler suites green in isolation (incl. resume-recovery regression), full server suite 2854/2855 (the lone gap a non-deterministic vitest worker-exit load-flake, disk fine). |
| 2026-06-20 | agent.service.launch decomposed (CC 25→14) | Reduced the repo's most delicate function (detached spawn, Windows shell, hot-reload survival, stdout-to-file, PID persistence) WITHOUT touching any spawn/fd/watcher call. Pure, historically bug-prone logic → tested leaf `lib/agent-launch-env.ts`: `shouldDetachAgent` (win32+shell rule), `resolveLaunchPorts` (board vs worktree port precedence), `buildAgentSpawnEnv` (full env wiring incl. KANBAN_PROTECTED_PIDS append + extraEnv-last). Side-effecting blocks → verbatim in-file helpers `writeInitialStdin`/`setupChildOutput`/`attachProcessHandlers` + shared `closeSessionWatchers` (de-dups exit/error cleanup); the fd/stdio open block stays inline (lifecycle tied to spawn). launch CC 25→14; +18 unit tests; existing agent.service.test (incl. env assertions) green. Verified `tsc` 0, `lint:arch` 0, full server suite 2867/2867. |
| 2026-06-20 | Client hotspot: AgentGrid view-model extraction | Took on the client module (highest avg risk). AgentGrid.tsx held the client's highest-CC functions (FeaturedCard 42, CompactCard 26) — dense conditional render driven by pure logic. Extracted a pure tested view-model `lib/agentGridView.ts` (resolveCardConfig, summarizeTodos, resolveContextTokens, buildDisplayHistory, resolveActivityText, selectVisibleAgents, partitionAgents, computeAgentCounts, computeEmptySlotCount, computeGridSizing — the plan-only-noise filter / attention detection / sort / sizing now unit-testable, +19 tests) and split the card JSX into focused sub-components (ActivityFeed, TodoBanner, FeaturedFooterStats, CompactFooterStats, GridHeader, EmptyState), markup verbatim. FeaturedCard 42→8, CompactCard 26→6. Verified: vite build green, no new tsc errors, lint:arch 0, full client suite 60 files / 529 tests + 19 new green; live /agents DOM-verified (all sections render, 0 console errors). Note: app-wide unstyled dev render = pre-existing Tailwind oxide WASI fallback (env), unrelated to this change. |
| 2026-06-20 | Tailwind oxide fix + TimelineView refactor | Fixed the dev-env Tailwind oxide WASI fallback (the win32-x64-msvc binding dir had the .node but no package.json → require failed → WASI → 0 utilities → unstyled app); restored package.json from the arm64 sibling, restarted dev server (app.css 10KB→263KB, UI styled). Then refactored TimelineView (client CC 23): pure view-model → tested `lib/timelineView.ts` (computeLanes/computeBaseRange/computeTicks/pctOf/toggleTypeSet/computeIssueBar, +15 tests), render split into sub-components (TimelineToolbar, GridLines [shared, was dup'd 3x], TimelineLane, TimelineTooltip). TimelineView CC 23→8. Verified: vite build, tsc (no new), lint:arch 0, client 61 files/544 tests green, /timeline visually verified styled. |
| 2026-06-20 | Client hotspots: TableView + CreateWorkspaceForm | Continued the client view-model extraction pass. **TableView**: `TableRow` (CC 17) badge/label lookups + overdue-date IIFE → pure tested `lib/tableView-cells.ts` (`resolveRowCells` + `isOverdue` with injectable now; +9 tests); TableRow CC 17→6; /table visually verified styled. **CreateWorkspaceForm**: `handleSubmit` (CC 23) POST-body construction → pure tested `lib/createWorkspaceBody.ts` (`parseProfileSelection`, `resolveDefaultProfile`, `buildCreateWorkspaceBody`; +11 tests); handleSubmit CC 23→10 (rest is async preflight/launch). Both: vite build green, no new tsc, lint:arch 0, client 63 files/564 tests green. |
| 2026-06-20 | Client: CreateIssuePanel payload + profile de-dup | Extracted `lib/createIssuePayload.ts` (`buildCreateIssuePayload`, launch fields gated on `start`; +8 tests) and de-duplicated the profile parse/default-resolution that was copy-pasted in both create forms — CreateIssuePanel's inline `profileSelection()` now reuses the shared `parseProfileSelection`/`resolveDefaultProfile` from `createWorkspaceBody.ts`. handleSubmit CC→5. vite build green, no new tsc, lint:arch 0, client 64 files/572 tests green. IssueDetailPanel/BoardToolbar already hook/registry-decomposed (their lizard CC is render-JSX artifact, not extractable pure logic). |
| 2026-06-20 | Client: WorkspacePanel launch-eligibility seam | Gave the worst-risk client file (WorkspacePanel) a tested pure seam: `lib/workspaceLaunchState.ts` (`detectQuickLaunchProvider`, `canResumeWorkspace`/`canRestartWorkspace` over a RelaunchContext — status gate, in-flight guard, provider-session-id presence; sessions typed structurally to keep the lib a leaf with no up-import; +7 tests). Behaviour preserved, no markup change. vite build green, no new tsc, lint:arch 0, client 65 files/579 tests green. |
| 2026-06-20 | Client: repo's two worst CC functions (registry + keymap) | Took on the #1 and #2 highest-CC functions in the entire repo, both untouched giant conditional cascades. **(1) `TerminalEventRenderer.renderParsedEvent` CC 142→1** (the single worst function): the 421-line if-cascade over `event.kind` became a typed per-kind renderer **registry**. `deriveRenderState()` computes the shared view flags once; a mapped `RendererRegistry = { [K in DisplayEvent["kind"]]: KindRenderer<K> }` dispatch table routes to one small renderer per kind, making the table **exhaustive by construction** (a new DisplayEvent kind fails to compile until registered — open/closed). The Agent tool_result branch split into its own renderer. Every JSX body sliced **byte-identically** via a Node brace-matcher (verified by content-signature + emoji diff vs original); peak logic-renderer CC ~9. **(2) `useBoardKeyboardShortcuts.handleKeyDown` CC 136→63**: extracted the arrow/vim cursor-navigation (nested column scan, the riskiest logic, into a **pure tested** `lib/boardKeyboardNav.ts` `computeNavTarget`, +6 edge/clamp/skip-empty tests — the hook had zero tests), and collapsed the ~11 uniform single-key toggles into a **declarative keymap** (`simpleBindings` match+run table dispatched by one guard-owning loop; adding a shortcut is now one line). Stateful g/g-s chord, Escape fall-through, Enter, command palette, dual-key c/w stay explicit. Verified: vite build, client tsc (no new errors), `lint:arch` 0 (888 modules), full client lib/hooks suite 498 green + 6 new nav tests. |
| 2026-06-20 | Server hotspot: computeInsights aggregation extracted | Continued the pure-tested-seam campaign onto the biggest server-logic function, `insights.service.computeInsights` (CC 90, the 3rd-worst function repo-wide after the two client cascades). The 150-line single-pass per-row aggregation (skill/model/issue-type/priority/provider-profile/time-series/top-expensive/context-leaderboard/friction roll-ups) extracted into a pure `accumulateInsightsRow` + `InsightsAccumulator` (`createInsightsAccumulator` seeds it; `AccumulateContext` carries the window-dependent inputs). computeInsights is now fetch → fold-each → destructure → finalize. Aggregation body transplanted **verbatim** (14 logic-signature markers verified unchanged); only accumulator counters write through the shared object; finalization byte-identical. computeInsights CC 90→33; aggregation isolated at CC 58 behind the new seam. +5 unit tests (totals, model/issue-type/priority buckets, friction roll-up, 7-day context window, unparseable-stats), the 17 existing insights integration tests still green; tsc clean, lint:arch 0. |
| 2026-06-20 | Server hotspot: session broadcast cascade decomposed (CC 63→16) | Took on the #1 remaining real-logic function, `session-manager/broadcast.ts` broadcast (CC 63) — the critical agent-stdout streaming path. Two phases under a fresh test net. **Phase 1:** extracted the entire 150-line per-stream-event `if (evt.X)` cascade verbatim (15 logic-signature markers verified unchanged) into a standalone, exported `applyStreamEvent(state, options, sessionId, evt)` — the guards stay intact so no narrowing breaks; broadcast drops to a thin buffer→persist→loop→exit→WS-deliver shell. Wrote a 9-test characterization suite (`apply-stream-event.test.ts`) feeding synthetic `ParsedStreamEvent`s and asserting state mutations + onLiveStats/onActivity/onTodos callbacks (substantive-output flag, assistant-text accumulation, ExitPlanMode/turn-complete, live stats, tool activity + Agent-id tracking, TodoWrite-vs-Task precedence, TaskCreate/Update, tool_result subagent decrement). **Phase 2:** under that net, split the four big sub-blocks into `applyStatsEvent`/`applyLiveStats`/`applyToolActivity`/`applyToolResult` (typed on `NonNullable<ParsedStreamEvent[...]>`) and the 8-condition substantive check into an `isSubstantiveEvent` predicate. Result: broadcast 63→16, applyStreamEvent 42→16, every handler ≤14; the cascade is now unit-testable in isolation. Verified: tsc clean, lint:arch 0 (888 modules), broadcast-batch (6) + apply-stream-event (9) suites green. |
| 2026-06-20 | Server hotspot: CLI reviewer-fixes analysis extracted (CC 62→25) | Next-worst real-logic function after broadcast: the `session reviewer-fixes` CLI action (CC 62, 254 NLOC) — correctness-sensitive review-effectiveness analysis tangled inline with DB/git I/O and text formatting, untested. Extracted the entire pure core into `lib/review-effectiveness-report.ts`: `classifyTriggerType`, `buildReviewWorkspaces` (epoch-ms session windows, closed so they don't overlap), `attributeCommits`/`buildReviewResult` (strict timezone-safe author-time attribution — the tz-offset bug magnet, since git `%aI` carries a local offset vs UTC session stamps), `summarizeReviewEffectiveness` (git/deep method + method-agreement rollup), `hasPositiveSeverity` (moved out of the command) + `computeDeepReviewSignals` (transcript heuristic). Handler keeps only I/O + rendering. +13 unit tests (tz-offset attribution edge case, window trimming, severity-negation heuristic, deep-signal combinations). Handler CC 62→25; tsc clean, lint:arch 0 (889 modules); both formatted + `--json` output paths smoke-tested end-to-end against the live board. |
