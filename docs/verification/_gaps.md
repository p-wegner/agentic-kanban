...
- **[partial]** `agent-sessions.launch.safety-guards` _(missing: config, regression)_ — spark refusal, default gpt-5.5, and preflight-block are asserted; the cross-provider-model DROP (a stored gpt-5.5 stripped from a Claude launch — the #698/#696 multi-cycle stall) is NOT directly asserted, only its downstream model-error lau
...
- **[partial]** `butler.reject.busy` _(missing: error-handling, api)_ — only the service-layer sendButlerTurn=false is asserted; the HTTP-level 409 on POST /message while busy is never asserted by any test
