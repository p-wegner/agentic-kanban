...
| `KANBAN_VERIFY_CHAIN_BACKGROUND_MAX_WAIT_MS` | How long a `background`-priority waiter (today: the base-health probe) may be overtaken by arriving gates before the verify-chain queue reverts to strict arrival order (#978). Default 30 min. Priority without a starvation bound is how a background job waits forever — merges arrive in bursts here, so each new gate would jump the probe indefinitely and the base's health would silently stop being measured, which is the failure the probe exists to prevent. Past the bound the worst case is a delay, never starvation. A non-numeric or negative value falls back to the default. |
| `KANBAN_TYPECHECK_WORKERS` | How many packages `scripts/typecheck.mjs` typechecks concurrently (#980). Default **2**, deliberately low rather than one-per-core: each `tsc` peaks around 0.5–1 GB and this box routinely runs several agents plus their builders, so a worker-per-core default is how one run takes the machine down. Measured on an idle box: 54s serial → 37s cold → ~10s warm. |
