...
| `KANBAN_TEMP_ENTRY_CAP` | Entry ceiling for the pre-merge gate's bounded `%TEMP%` health probe (`shared/lib/temp-health.ts`, #1056). Default **50,000**. Above it the gate HOLDS instead of starting a verify chain: measured, a `%TEMP%` at 707,242 entries made a bare directory enumeration exceed 120s, so every process that touches it stalls — which a test runner does constantly. That is the state that killed four gate runs across three branches with **no test output at all** and drew three successive wrong diagnoses (machine load, then log truncation, then the board's own sweeper). A hold is NOT a red gate: nothing ran and nothing failed, and the merge retries next cycle. The probe never walks the whole directory — it stops at this cap or at `KANBAN_TEMP_PROBE_BUDGET_MS`, because counting the directory is the very operation that stalls. A malformed or non-positive value is IGNORED, not honoured. The server's vitest setup raises it (`test-setup/temp-health-neutral.ts`) so a unit test of what the gate does downstream is not at the mercy of the developer's own temp dir. |
| `KANBAN_TEMP_PROBE_BUDGET_MS` | Wall-clock ceiling for that same probe. Default **2000**. Reaching it without finishing is itself the degraded verdict — arrived at from the other side — and it is what guarantees the preflight can never cost more than it saves: at most this many ms against the ~28-minute gate run it decides whether to start. |
