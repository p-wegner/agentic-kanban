...
| `KANBAN_BASE_HEALTH_MAX_CONSECUTIVE_YIELDS` | How many times in a row a RUNNING base-health probe may abandon itself at a stage boundary because a merge gate is queued behind it (#989); the next one then runs to completion. Default 3. `0` disables preemption entirely (the pre-#989 behaviour). A yielded probe records nothing but logs and counts the yield — the streak clears on any completed run. |
