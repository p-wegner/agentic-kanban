...
4. KEEP THE BACKLOG ABOVE BACKLOG_FLOOR. If the backlog has fewer than BACKLOG_FLOOR items, run the $backlog-refill skill to create enough new tickets to get back above the floor — weighted toward substantial features with some quality work mixed in. Act early; never let it hit zero. Newly created tickets feed priority 3 (they get pulled into the sprint and run on later cycles).
